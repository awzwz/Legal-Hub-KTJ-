...
const StatCard = ({ icon: Icon, label, value, sub, trend, variant = "default", delay = 0, footer }: {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-xl border border-blue-100 p-5 shadow-sm hover:shadow-md transition-shadow min-w-0 overflow-hidden"
    >
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        <StatCard icon={DollarSign} label="Общая сумма" value={formatAmount(defendantClaim)} sub={formatAmountShort(defendantClaim)} variant="warning" delay={0.3} />
        <StatCard
          icon={DollarSign}
          label="Оплачено"
          value={formatAmount(defendantPaid)}
          variant="success"
          sub={
            defendantClaim > 0
              ? `${formatAmountShort(defendantPaid)} · ${((defendantPaid / defendantClaim) * 100).toFixed(1)}% погашено`
              : formatAmountShort(defendantPaid)
          }
          delay={0.35}
        />
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3">
        <StatCard icon={DollarSign} label="Остаток долга" value={formatAmount(plaintiffRemaining)} variant="overdue" sub={formatAmountShort(plaintiffRemaining)} delay={0.4} />
      </div>
    </div>
  );
};
