import { mockCases, caseStatusLabels, courtInstanceLabels, partyRoleLabels, formatAmount, type LegalCase } from "@/data/mockData";
...
const statusStyles: Record<string, string> = {
  ...
  execution: "bg-blue-100 text-blue-700 border border-blue-200",
...
const CasesTable = ({ onCaseClick, cases }: CasesTableProps) => {
  const displayCases = cases || mockCases;
            ...
            {overdueCases.map(c => (
              ...
              >
                <p className="text-blue-600 group-hover:text-blue-800 transition-colors truncate">
                  <span className="font-medium text-blue-900">{c.caseNumber}</span> — {c.company} — <span className="text-red-600 font-medium">{c.daysOverdue} дн. просрочки</span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-blue-100 bg-blue-50/50">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-blue-700">Компания</th>
            ...
            <tbody>
              {displayCases.map((c, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div>
                      <p className="font-medium truncate max-w-[180px] text-blue-900">{c.company}</p>
                      <p className="text-xs text-blue-500">БИН: {c.companyBIN}</p>
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs">{c.caseType === "civil" ? "Гр." : c.caseType === "administrative" ? "Адм." : c.caseType === "criminal" ? "Уг." : "Исп."}</td>
