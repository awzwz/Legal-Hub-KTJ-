...
Дефолтные креды демо-сидера доступны только при `DEMO_SEED_ENABLED=true` в локальном или
одноразовом тестовом окружении. Production-конфигурация запрещает включать этот режим.
...
> Не используйте demo-аккаунты на сервере с реальными данными. Для production создайте
> персональные учётные записи отдельной процедурой.
