import { getCounterparties, formatAmount, formatAmountShort, mockCases, getFilteredCasesForUser, canViewAllCases, caseStatusLabels, courtInstanceLabels, caseTypeLabels } from "@/data/mockData";
...
const CounterpartiesPage = ({ onCaseClick }: CounterpartiesPageProps) => {
  ...
  const counterparties = useMemo(() => {
    const map = new Map<string, { bin: string; name: string; totalCases: number; activeCases: number; totalDebt: number; totalPaid: number; lastCaseDate: string }>();
    userCases.forEach(c => {
  ...
  const detailCases = detailBIN
    ? mockCases.filter(c => c.companyBIN === detailBIN && (canViewAll || c.branch === user.branch))
    : [];
              ...
              <div className="stat-card">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <a
                      href={`https://adata.kz/company/${cp.bin}`}
                      target="_blank"
                ...
                {isExpanded && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="mt-3 pt-3 border-t space-y-2">
                    {mockCases.filter(c => c.companyBIN === cp.bin && (canViewAll || c.branch === user.branch)).map(c => (
                      <div
                        key={c.id}
            ...
            <>
              <DialogHeader>
                ...
                <div className="flex items-center gap-3 mt-1">
                  ...
                  <a
                    href={`https://adata.kz/company/${detailCP.bin}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  >
