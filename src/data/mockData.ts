...
export const getLawyerStats = () => {
  return lawyers.map(lawyer => {
    ...
    return { name: lawyer, totalCases: cases.length, won, lost, active, totalAmount, avgDays, winRate: cases.length > 0 ? Math.round((won / Math.max(won + lost, 1)) * 100) : 0 };
  }).sort((a, b) => b.winRate - a.winRate);
};

// Role system
export type UserRole = "director" | "branch_lawyer" | "accountant";
...
export let currentUser: User = getStoredUser();
...
export const canViewLawyerStats = (user: User): boolean => user.role === "director";
...
export const getFilteredCasesForUser = (user: User, cases: LegalCase[] = mockCases): LegalCase[] => {
